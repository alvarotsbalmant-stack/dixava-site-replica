...
export const useBackgroundRemovalLazy = () => {
  ...
  const processImageFromUrl = async (
    ...
    if (!backgroundRemovalModule) {
      toast.error('Módulo de remoção de fundo não carregado');
      return null;
    }
    ...
    try {
      ...
      const imageElement = await backgroundRemovalModule.loadImageFromUrl(imageUrl);
      setProgress(30);
      
      console.log('📷 Imagem carregada, iniciando remoção de fundo...');
      
      // Remover fundo com opções avançadas
      const processedBlob = await backgroundRemovalModule.removeBackground(imageElement, options);
      setProgress(80);
  ...
  const processImageFromFile = async (
    ...
    try {
      ...
      const imageElement = await backgroundRemovalModule.loadImage(file);
      setProgress(30);
