-- Atualizar função de teste para implementar perda de streak
...
BEGIN
  ...
  IF v_streak.last_login_date IS NOT NULL THEN
    ...
    -- Se passou mais de 2 intervalos sem resgatar, perdeu o streak
    IF last_bonus_available_time IS NOT NULL 
       AND NOW() > (last_bonus_available_time + (test_cooldown_seconds || ' seconds')::INTERVAL) THEN
      missed_bonus := true;
    END IF;
  END IF;
