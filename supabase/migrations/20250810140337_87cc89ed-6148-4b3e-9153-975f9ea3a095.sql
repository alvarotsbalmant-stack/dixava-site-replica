-- Implementação Corrigida do Sistema de Códigos Diários com Streak de 48h
...
-- 6. Índices básicos para performance (sem predicados com NOW())
CREATE INDEX IF NOT EXISTS idx_daily_codes_code ON daily_codes(code);
CREATE INDEX IF NOT EXISTS idx_daily_codes_created_at ON daily_codes(created_at);
CREATE INDEX IF NOT EXISTS idx_daily_codes_claimable_until ON daily_codes(claimable_until);
CREATE INDEX IF NOT EXISTS idx_daily_codes_valid_until ON daily_codes(valid_until);
CREATE INDEX IF NOT EXISTS idx_user_codes_user_id ON user_daily_codes(user_id);
CREATE INDEX IF NOT EXISTS idx_user_codes_expires_at ON user_daily_codes(expires_at);
