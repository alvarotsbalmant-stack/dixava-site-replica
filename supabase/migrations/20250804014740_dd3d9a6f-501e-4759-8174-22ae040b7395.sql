-- Phase 1: Fix Test SQL Functions with correct JSONB casting

-- Fix can_claim_daily_bonus_test function
...
BEGIN
  -- Buscar configuração de cooldown de teste com JSONB casting correto
  SELECT (setting_value #>> '{}')::INTEGER INTO test_cooldown_seconds
  FROM coin_system_config 
  WHERE setting_key = 'test_cooldown_seconds';
  
  test_cooldown_seconds := COALESCE(test_cooldown_seconds, 10);
...
-- Fix process_daily_login_test function
...
DECLARE
  ...
  test_cooldown_seconds INTEGER;
  last_bonus_available_time TIMESTAMP WITH TIME ZONE;
  missed_bonus BOOLEAN := false;
BEGIN
  -- Verificar se pode resgatar no modo de teste
  SELECT * INTO claim_info FROM public.can_claim_daily_bonus_test(p_user_id);
  
  -- Buscar configuração de cooldown com JSONB casting correto
  SELECT (setting_value #>> '{}')::INTEGER INTO test_cooldown_seconds
  ...
  IF NOT claim_info.can_claim THEN
    RETURN jsonb_build_object(
      'success', false, 
      'message', format('Aguarde %s segundos antes do próximo resgate (modo teste)', test_cooldown_seconds),
      'last_claim', claim_info.last_claim,
      'next_reset', claim_info.next_reset
    );
  END IF;
  
  -- Buscar configurações do daily bonus com JSONB casting correto
  ...
  -- Verificar se perdeu algum bônus (lógica de perda de streak no modo teste)
  IF v_streak.last_login_date IS NOT NULL THEN
    -- Calcular quando o último bônus deveria ter ficado disponível
    SELECT last_performed_at + (test_cooldown_seconds || ' seconds')::INTERVAL 
    INTO last_bonus_available_time
    FROM daily_actions 
    WHERE user_id = p_user_id 
      AND action = 'daily_login'
    ORDER BY last_performed_at DESC
    ...
    -- Se passou mais de 2 intervalos sem resgatar, perdeu o streak (teste: 2x cooldown)
    IF last_bonus_available_time IS NOT NULL 
       AND NOW() > (last_bonus_available_time + (test_cooldown_seconds * 2 || ' seconds')::INTERVAL) THEN
      missed_bonus := true;
    END IF;
  END IF;
  
  IF NOT FOUND OR missed_bonus THEN
    -- Primeiro login ou perdeu streak
    INSERT INTO user_streaks (user_id, current_streak, longest_streak, last_login_date, streak_multiplier)
    VALUES (p_user_id, 1, GREATEST(COALESCE(v_streak.longest_streak, 0), 1), CURRENT_DATE, 1.0)
    ON CONFLICT (user_id)
    DO UPDATE SET
      current_streak = 1,
  ...
  VALUES (p_user_id, v_final_amount, 'earned', 'daily_login', 
          'Daily Bonus (modo teste)', jsonb_build_object(
            'test_mode', true, 
            'streak_day', v_new_streak, 
            'total_streak_days', v_streak_days, 
            'increment_type', v_increment_type,
...
-- Also fix the production function that has similar issues
...
-- Enable test mode and set correct test cooldown
INSERT INTO coin_system_config (setting_key, setting_value, description)
VALUES ('test_mode_enabled', 'true', 'Enable test mode for daily bonus system')
...
INSERT INTO coin_system_config (setting_key, setting_value, description)
VALUES ('test_cooldown_seconds', '10', 'Cooldown in seconds for test mode daily bonus')
ON CONFLICT (setting_key) 
DO UPDATE SET 
  setting_value = '10',
  updated_at = now();