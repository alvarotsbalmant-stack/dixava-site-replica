-- Corrigir função para usar daily_login_amount instead of base_bonus_amount
...
BEGIN
  ...
  -- Usar daily_login_amount como preferência, fallback para base_bonus_amount
  SELECT (setting_value)::INTEGER INTO v_base_amount
  FROM coin_system_config WHERE setting_key = 'daily_login_amount';
  ...
  VALUES (p_user_id, v_final_amount, 'earned', 'daily_login', 
          'Login diário (modo teste)', jsonb_build_object('test_mode', true, 'streak', v_new_streak, 'multiplier', v_new_multiplier, 'base_amount', v_base_amount));
