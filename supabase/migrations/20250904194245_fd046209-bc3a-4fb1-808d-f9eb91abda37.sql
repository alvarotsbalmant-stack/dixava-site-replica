-- Atualizar função complete_order_verification para incluir cashback em UTI Coins
...
BEGIN
  ...
  -- CALCULAR CASHBACK EM UTI COINS
  ...
  LOOP
    ...
    SELECT uti_coins_cashback_percentage INTO v_product_cashback
    FROM public.products 
    WHERE id = v_product_id;
    
    -- Se o produto tem cashback configurado
    ...
    IF v_cashback_amount > 0 THEN
      PERFORM public.earn_coins(
        ...
        format('Cashback da compra (%.2f%% = R$ %.2f) - Código: %s', 
               (v_cashback_amount::NUMERIC / (v_total_cashback * 100) * 100), 
               v_total_cashback, 
               p_code),
        jsonb_build_object(
          'cashback_reais', v_total_cashback,
